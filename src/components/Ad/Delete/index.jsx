...
import { fetchAllNotifications } from "@/features/notifications/notificationsApi";
...
export default function DeleteNotificationModal({
  ...
  const { removeNotification, reset, successMessage, error, isLoading } =
    useNotifications();
  ...
  useEffect(() => {
    if (successMessage) {
      ...
      dispatch(fetchAllNotifications({ pageNo }));
      closeModal();
    }
    ...
    <ModalLayout isOpen={show} setIsOpen={closeModal}>
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-auto p-6">
        <div className="flex justify-between items-center border-b border-gray-300 pb-2 mb-4">
          <h3 className="md:text-lg font-semibold">Delete Notification</h3>
